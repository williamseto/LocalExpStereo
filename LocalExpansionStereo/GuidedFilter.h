...
public:
    ...
    cv::Mat filter_raw(const cv::Mat& p) const
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            auto pI_r = Ichannels[0].template ptr<Type>(i);
            auto pI_g = Ichannels[1].template ptr<Type>(i);
            auto pI_b = Ichannels[2].template ptr<Type>(i);
...
{
protected:
    // bring base case member vars into scope, and make dependent
    using GuidedImageFilter<Type>::Ichannels;
    using GuidedImageFilter<Type>::I;
    using GuidedImageFilter<Type>::R;
    using GuidedImageFilter<Type>::eps;
    using GuidedImageFilter<Type>::realI;

    using GuidedImageFilter<Type>::mean_I_r;
    using GuidedImageFilter<Type>::mean_I_g;
    using GuidedImageFilter<Type>::mean_I_b;

    using GuidedImageFilter<Type>::invrr;
    using GuidedImageFilter<Type>::invrg;
    using GuidedImageFilter<Type>::invrb;
    using GuidedImageFilter<Type>::invgg;
    using GuidedImageFilter<Type>::invgb;
    using GuidedImageFilter<Type>::invbb;
